    ...
    public class Adnane
    {
        ...
        public static void TriTableau(int[] Tableau, int taille_Tableau)
        {
            ...
            for (int i = 0; i < taille_Tableau - 1; i++)
            {
                ...
                DoubleChanger(Tableau[Position_Mini], Tableau[i]);
            }
        ...
        public static bool TableauCroissant(int[] Tableau, int taille_Tableau)
        {
            int i = 0;
            bool Flag = true;
        ...
        static int N = 9;
        public static bool solveSudoku(int[,] grid, int ligne, int colonne)
        {
            ...
            {
                return solveSudoku(grid, ligne, colonne + 1);
            }
            for (int num = 1; num < 10; num++)
            {
                if (isSafe(grid, ligne, colonne, num))
                {
                    grid[ligne, colonne] = num;
                    if (solveSudoku(grid, ligne, colonne + 1))
                    {
                        return true;
                    }
                }
        ...
        public static bool isSafe(int[,] grid, int ligne, int colonne, int num)
        {
            for (int x = 0; x <= 8; x++)
            {
                if (grid[ligne, x] == num)
